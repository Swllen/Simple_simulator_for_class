...
# Compute I
def compute_irradiance(optical_sys, detector_surface=-1, res=(256, 256), num_rays=100000):
...
# Compute the center of spot (Also can add mask)
def compute_centroid(irr_map, x_edges, y_edges, mask_radius=None):
    """
    Internal helper: Compute spot centroid with optional masking.
    
    Args:
        irr_map: Irradiance map tensor
        x_edges: X coordinate edges
        y_edges: Y coordinate edges
        mask_radius: If provided, apply circular mask before computing centroid
        
    Returns:
        tuple: (xc, yc, total_power)
    """
    if mask_radius is not None:
        return spot_centroid_from_irr_mask(irr_map, x_edges, y_edges, mask_radius=mask_radius)
    else:
        return spot_centroid_from_irr(irr_map, x_edges, y_edges)

# Detector-Size -> get edges in order to calculate position, power and so on
def get_detector_edges(detector_size):
    """Get Detector physical edges"""
    detector_half_width = detector_size[0]  # mm
    detector_half_height = detector_size[1]  # mm
    x_edges = np.linspace(-detector_half_width, detector_half_width, 257)
    y_edges = np.linspace(-detector_half_height, detector_half_height, 257)
    return x_edges, y_edges

def normalize_irradiance(irr_map):
    """Normalize irradiance."""
    if not torch.is_tensor(irr_map):
        irr_map = torch.tensor(irr_map, dtype=torch.float32)
    
    if torch.max(irr_map) > 0:
         return irr_map  / torch.max(irr_map)
    else:
        return irr_map
...
def compute_irr(optical_sys, detector_surface=-1,
    ...
    """
    irr_map, _, _ = compute_irradiance(
    ...
    if normalize:
        irr_map = normalize_irradiance(irr_map)
...
def compute_irr_in_mask(optical_sys, detector_surface=-1, radius=1.0,
    ...
    mask = create_circular_mask(x_edges, y_edges, radius, res=res, device=device, center=center, invert=False)
    
    valid_irr_map = irr_map * mask

    return valid_irr_map, mask

# Compute I with mask of pinhole -> Compute power
def compute_power(optical_sys, detector_surface=-1, radius=1, 
    ...
    mask = create_circular_mask(x_edges, y_edges, radius, res=res, device=device, invert=False)
...
def spot_centroid_from_irr(irr_map, x_edges, y_edges, tau=0.05, eps=1e-12, 
    ...
    # Meshgrids matching irr_map shape (W, H)
    # X, Y = torch.meshgrid(x_centers, y_centers, indexing='ij')
    X = x_centers.unsqueeze(0).expand_as(irr_map)   # (H, W)
    Y = y_centers.unsqueeze(1).expand_as(irr_map)   # (H, W)
...
# Compute I with mask of pinhole -> Center
def spot_centroid_from_irr_mask(irr_map, x_edges, y_edges, mask_radius=1.0, 
    ...
    if mask_radius is not None:
        device = irr_map.device
        mask = create_circular_mask(x_edges, y_edges, radius=mask_radius, res=res, invert=False, device=device)
...
def compute_irr_with_aperture_viz(optical_sys, detector_surface=-1,
                                   ...
                                   show_plot=True, linewidth=1, linestyle='-',
