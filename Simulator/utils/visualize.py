...
def save_3d_snapshot(sys, filename="optic_view.png", width=800, height=600, dark_mode=False, **kwargs):
    ...
    camera.SetPosition(0, 0, -10)     # camera location
    ...
    **kwargs
    ...
    for i, p in enumerate(params_log):
        ...
        if save_irr:
            ...
            irr_filename = irr_dir / f'iter_{int(round(iteration)):04d}_irradiance.png'
        ...
        if save_3d:
            try:
                vis_3d_filename = vis_3d_dir / f'iter_{int(round(iteration)):04d}_3d_view.png'
